...
namespace AdeptusMechanicus
{
        ...
        public void initalizeGraphics(ThingDef parentDef)
        {
            foreach (var entry in PauldronEntries)
            {
                if (!entry.padTexPath.NullOrEmpty())
                {
                    ...
                    if (idx != -1)
                    {
                        string s1 = path.Substring(0, idx);
                        /*
                        Log.Message(s1);
                        string s2 = path.Substring(idx + 1);
                        Log.Message(s2);
                        */
                        path = s1;
                     //   Log.Message(path);
                    }
                    List<Texture2D> mainList = ContentFinder<Texture2D>.GetAllInFolder(path).ToList();
    ...
    public class CompPauldronDrawer : ThingComp, ILoadReferenceable
    {
        ...
        public void Initialize()
        {
            ...
            {
                Log.Message(this.parent.LabelShortCap + " pauldrons initializing");
...
#pragma warning restore CS0219 // Variable is assigned but its value is never used
            ...
            {
                Log.Message(this.parent.LabelShortCap + " pauldrons initialized");
                UpdateActive();
                if (Props.drawAll)
                {
                    foreach (ShoulderPadProperties props in Props.PauldronEntries)
                    {
                        ShoulderPadEntry entry = activeEntries.FirstOrFallback(x => x.Props == props);
                        if (entry == null)
                        {
                            entry = new ShoulderPadEntry(props, this);
        ...
        */
        ...
        const float _HeadOffset = 0f + MinClippingDistance;
        const float _HairOffset = 0244897958f + MinClippingDistance;       // Number must be same as PawnRenderer.YOffset_Head
        const float _BodyOffset = 0f + MinClippingDistance;   // Number must be same as PawnRenderer.YOffset_Shell
