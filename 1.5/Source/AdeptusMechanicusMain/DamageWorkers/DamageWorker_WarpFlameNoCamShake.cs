    ...
    public class DamageWorker_WarpFlameNoCamShake : DamageWorker_AddInjury
    {
        public override DamageWorker.DamageResult Apply(DamageInfo dinfo, Thing victim)
        {
            ...
            if (victim.Destroyed && map != null && pawn == null)
            {
                ...
                if (plant != null && victim.def.plant.IsTree && plant.LifeStage != PlantLifeStage.Sowing && victim.def != AdeptusThingDefOf.BurnedTree)
                {
                    DeadPlant deadPlant = (DeadPlant)GenSpawn.Spawn(AdeptusThingDefOf.BurnedTree, victim.Position, map, WipeMode.Vanish);
