...
namespace AdeptusMechanicus
{
    ...
    public class TunnelSpawner : ThingWithComps, IThingHolder
    {
        public TunnelSpawner()
        {
        ...
        {
            TunnelSpawner.filthTypes.Clear();
            TunnelSpawner.filthTypes.Add(ThingDefOf.Filth_Dirt);
            TunnelSpawner.filthTypes.Add(ThingDefOf.Filth_Dirt);
            TunnelSpawner.filthTypes.Add(ThingDefOf.Filth_Dirt);
            TunnelSpawner.filthTypes.Add(ThingDefOf.Filth_RubbleRock);
        }
        ...
        public override void Tick()
        {
            if (base.Spawned)
            {
                ...
                IntVec3 c;
                Rand.PushState();
                if (Rand.MTBEventOccurs(TunnelSpawner.FilthSpawnMTB, 1f, 1.TicksToSeconds()) && CellFinder.TryFindRandomReachableCellNearPosition(base.Position, base.Position, base.Map, TunnelSpawner.FilthSpawnRadius, TraverseParms.For(TraverseMode.NoPassClosedDoors, Danger.Deadly, false), null, null, out c, 999999))
                {
        ...
        private void DrawDustPart(float initialAngle, float speedMultiplier, float scale)
        {
            ...
            TunnelSpawner.matPropertyBlock.SetColor(ShaderPropertyIDs.Color, value);
            Matrix4x4 matrix = Matrix4x4.TRS(pos, Quaternion.Euler(0f, initialAngle + speedMultiplier * num, 0f), Vector3.one * scale);
            Graphics.DrawMesh(MeshPool.plane10, matrix, TunnelSpawner.TunnelMaterial, 0, null, 0, TunnelSpawner.matPropertyBlock);
        }
