...
namespace AdeptusMechanicus
{
	public class Window_Reserves : Window
	{
		...
		{
			if (!WorldRendererUtility.WorldRenderedNow && ReservesTracker.nextReserve != null)
			{
				base.WindowOnGUI();
			}
		}
		...
		private void UpdatePosition()
		{
			...
			if (this.windowRect.y < 0f)
			{
				this.windowRect.y = 0f;
			}
			if (this.windowRect.x > (float)UI.screenWidth)
			{
		...
		{
			bool drawBackground = AMAMod.Dev ;// VESWWMod.settings.drawBackground;
			if (drawBackground)
			{
			...
			if (this.ReservesTracker.nextReserve != null && this.ReservesTracker.nextReserve.deployed && !this.ReservesTracker.nextReserve.defeated)
			{
		...
		*/
		...
		//	this.DoWaveNumberAndModifierUI(rect3);
            ...
            Rect rect4 = rect2;
			Widgets.Label(rect4, this.ReservesTracker.nextReserve?.TimeBeforeDeployment() ?? "No Reserves");
            ...
            if (showPawnList)
            {
                ...
                Rect rect6 = rect2;
				Widgets.Label(rect6, this.ReservesTracker.nextReserve.Members.Select(x=> x.KindLabel).ToCommaList());
				yMax = rect6.yMax;
            }
            rect2 = new Rect(rect)
			{
            ...
            Rect rect7 = rect2;
			if (Widgets.ButtonText(rect7, "Deploy Now"/*.Translate()*/, true, true, true))
			{
		...
		//	this.DoWaveNumberAndModifierUI(rect3);
            ...
            Rect rect4 = rect2;
            ReserveForce next = this.ReservesTracker?.nextReserve;
            if (next?.Lords != null && next?.Members.Count>0)
            {

                int num = next.Members.Count - next.Members.Where(x=> x.Spawned && !x.Dead && !x.Downed).Count();
				this.DrawFillableBar(rect4, string.Format("{0}/{1}", num, next.Members.Count), (float)num / (float)next.Members.Count, true);
                rect2 = new Rect(rect)
				{
					x = rect3.xMax - 20f,
					y = rect4.yMax + 10f,
