    ...
    public static class JobGiver_AIFightEnemy_TryGiveJob_RangedVerb_Patch
    {
        ...
        static bool Prefix(ref JobGiver_AIFightEnemy __instance, ref Job __result, ref Pawn pawn)
        {
            ...
            List<Verb> rangeList = new List<Verb>();
            {
                ...
                Log.Message("Checking pawn.verbTracker.AllVerbs for Ranged Verbs");
                for (int i = 0; i < verbList.Count; i++)
                {
                    Log.Message($"verb: {i}, Name: {verbList[i].verbProps.label} RangeMax: {verbList[i].verbProps.range}, Projectile: {verbList[i].verbProps.LaunchesProjectile}");
                    //It corresponds with verbs anyway
                    if (verbList[i].verbProps.range > 1.5f)
                    {
                        rangeList.Add(verbList[i]);
                        hasRangedVerb = true;
                        Log.Message("Added Ranged Verb");
                    }
                }
            }
            if (pawn.equipment?.PrimaryEq!=null)
            {
                List<Verb> verbList = pawn.equipment.PrimaryEq.AllVerbs;
                Log.Warning("Checking pawn.equipment.PrimaryEq.AllVerbs for Ranged Verbs");
                for (int i = 0; i < verbList.Count; i++)
                {
                    Log.Message($"verb: {i}, Name: {verbList[i].verbProps.label} RangeMax: {verbList[i].verbProps.range}, Projectile: {verbList[i].verbProps.LaunchesProjectile}");
                    //It corresponds with verbs anyway
                    if (verbList[i].verbProps.range > 1.5f)
                    {
                        if (!rangeList.Contains(verbList[i]))
                        {
                            rangeList.Add(verbList[i]);
                            hasRangedVerb = true;
                            Log.Message("Added Ranged Verb");
            ...
            if (pawn.health.hediffSet.hediffs.Any(x => VerbGiver_hediffs.Contains(x.def) /*x.TryGetCompFast<HediffComp_VerbGiverExtra>() != null*/))
            {
                ...
                Log.Warning("Checking pawn.health.hediffSet.VerbGiver_hediffs for Ranged Verbs");
                foreach (Hediff item in list)
                {
                    HediffComp_VerbGiverExtra _VerbGiver = item.TryGetCompFast<HediffComp_VerbGiverExtra>();
                    if (_VerbGiver != null)
                    {
                        List<Verb> verbList = _VerbGiver.verbTracker.AllVerbs;
                        for (int i = 0; i < verbList.Count; i++)
                        {
                            Log.Message($"verb: {i}, Name: {verbList[i].verbProps.label} RangeMax: {verbList[i].verbProps.range}, Projectile: {verbList[i].verbProps.LaunchesProjectile}");
                            //It corresponds with verbs anyway
