                ...
                if (!potentialDeepStrikers.NullOrEmpty() || !potentialInfiltrators.NullOrEmpty())
                {
                    ...
                    //    Log.Message($"Arrive_DSI DeepStrike:{potentialDeepStrikers.Count} Deep Strike Capable");
                        ...
                        if (!deepStrikers.NullOrEmpty())
                        {
                            ...
                            Rand.PopState();
                            ReserveForce reserveForce = new ReserveForce(parms.faction, parms.lord, delay, map, deepStrikers);
                            map.Reserves().Reserves.Add(reserveForce);
                            
                            if (Find.Storyteller.def.defName == "VSE_WinstonWave")
                            {
                                Log.Message($"Pre Arrive_DSI DeepStrikers: {deepStrikers.Count} with Winston Waves arriving in {delay.TicksToSeconds()}");
                            }
                    ...
                    //    Log.Message($"Arrive_DSI Infiltrate: {potentialInfiltrators.Count} Infiltrators");
                        ...
                        if (!infiltrators.NullOrEmpty())
                        {
                            ...
                            ReserveForce reserveForce = new ReserveForce(parms.faction, parms.lord, delay, map, infiltrators);
                            map.Reserves().Reserves.Add(reserveForce);
                            
                            if (Find.Storyteller.def.defName == "VSE_WinstonWave")
                            {
                                Log.Message($"Pre Arrive_DSI Infiltrators: {infiltrators.Count} with Winston Waves arriving in {delay.TicksToSeconds()}");
                            }
