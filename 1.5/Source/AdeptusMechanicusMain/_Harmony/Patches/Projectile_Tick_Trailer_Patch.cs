                        ...
                        if (trailer != null)
                        {
                            if (___ticksToImpact % trailer.trailerMoteInterval == 0)
                            {
                                for (int ii = 0; ii < trailer.motesThrown; ii++)
                                {
                                    ...
                                    if (AMAMod.Dev) Log.Message($"TrailThrower.ThrowSprayTrail for: {__instance} {___ticksToImpact} {___origin} {___destination} Mote= {trailer.TrailMoteDef}");
                                
                                TrailThrower.ThrowSprayTrail(__instance.DrawPos, __instance.Map, ___origin, ___destination, trailer.TrailMoteDef, trailer.trailMoteSize, 240, __instance.def.projectile.SpeedTilesPerTick, DC);
                                }
                            }
                        }
                    }
