    ...
    class TestingScenarioUtility
    {
        public static void SetUpTestScenarios(List<ScenarioDef> scenariosTesting)
        {
            ...
            foreach (ScenarioDef ScenDef in scenariosTesting)
            {
                if (ScenDef.defName.Contains("Imperial"))
                {
                    ...
                    TryAddWeaponsToScenario(ScenDef, "IQ");
                    TryAddWeaponsToScenario(ScenDef, "OA");
                    TryAddWeaponsToScenario(ScenDef, "AS");
                }
        ...
        public static void TryAddWeaponsToScenario(ScenarioDef ScenDef, string Tag)
        {
            ...
            foreach (ThingDef Weapon in things)
            {
                ...
                foreach (ScenPart scenpart in parts.Where(x => x.def == AdeptusScenPartDefOf.StartingThing_Defined))
                {
                ...
                ScenPart_StartingThing_Define StartingThing = new ScenPart_StartingThing_Define() { def = AdeptusScenPartDefOf.StartingThing_Defined };
        ...
        public static void TryAddWeaponsToScenario(ScenarioDef ScenDef, List<ThingCategoryDef> Tags)
        {
            List<ThingDef> things = DefDatabase<ThingDef>.AllDefsListForReading.FindAll(x => x.PlayerAcquirable && x.thingCategories.Any(y=> Tags.Contains(y)) && !x.defName.Contains("TOGGLEDEF_S"));
            foreach (ThingDef Weapon in things)
            {
                bool hasweapon = false;
                List<ScenPart> parts = ScenDef.scenario.parts;
