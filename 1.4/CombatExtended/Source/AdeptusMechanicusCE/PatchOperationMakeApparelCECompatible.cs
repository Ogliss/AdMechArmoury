...
namespace AdeptusMechanicus
{
    public class PatchOperationMakeApparelCECompatible : PatchOperation
	{
				...
				else
				{
					...
					try
					{
						while (enumerator.MoveNext())
						{
							...
							if (this.layersPrepend != null && this.layersPrepend.node.HasChildNodes || this.layersAppend != null && this.layersAppend.node.HasChildNodes)
							{
								this.AddOrReplaceapparelLayer(xml, xmlNode);
							}
							...
							if (this.bodyPartGroups != null && this.bodyPartGroups.node.HasChildNodes)
							{
								AddOrReplacebodyPartGroups(xml, xmlNode);
							}
		...
		private void Populate(XmlDocument xml, XmlNode reference, ref XmlElement destination, bool overrideExisting = true, bool prepend = false)
		{
			...
			try
			{
				while (enumerator.MoveNext())
				{
					...
					if (prepend) destination.PrependChild(xml.ImportNode(xmlNode, true));
					else destination.AppendChild(xml.ImportNode(xmlNode, true));
				}
			}
			finally
			{
		...
		private void AddOrReplacebodyPartGroups(XmlDocument xml, XmlNode xmlNode)
		{
			XmlElement xmlElement;
			this.GetOrCreateNode(xml, xmlNode, "apparel/bodyPartGroups", out xmlElement);
			this.Populate(xml, this.bodyPartGroups.node, ref xmlElement, false);
		}
		private void AddOrReplaceapparelLayer(XmlDocument xml, XmlNode xmlNode)
		{
			XmlElement xmlElement;
			this.GetOrCreateNode(xml, xmlNode, "apparel/layers", out xmlElement);
			if (this.layersPrepend != null && this.layersPrepend.node.HasChildNodes)
			{
				this.Populate(xml, this.layersPrepend.node, ref xmlElement, false, true);
			}
			if (this.layersAppend != null && this.layersAppend.node.HasChildNodes)
			{
				this.Populate(xml, this.layersAppend.node, ref xmlElement, false, false);
			}
		}
		...
		public XmlContainer bodyPartGroups;
		public XmlContainer layersPrepend;
		public XmlContainer layersAppend;
