...
public static class DeepStrikeCellFinder
{
		...
		bool SpotValidator(IntVec3 c)
		{
			...
			if (ModsConfig.RoyaltyActive)
			{
				...
				for (int j = 0; j < list.Count; j++)
				{
					...
					{
                        CompSendSignalOnMotion compSendSignalOnPawnProximity = list[j].TryGetCompFast<CompSendSignalOnMotion>();
	...
	{
		if (!TryFindShipLandingArea(map, out IntVec3 result, out firstBlockingThing))
		{
