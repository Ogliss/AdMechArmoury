...
namespace AdeptusMechanicus
{
    ...
    public static class AdeptusLightningBoltMeshMaker
    {
        ...
        {
            Rand.PushState();
            AdeptusLightningBoltMeshMaker.lightningTop = new Vector2(Rand.Range(xMin, xMax), z);
            Rand.PopState();
            AdeptusLightningBoltMeshMaker.MakeVerticesBase();
            AdeptusLightningBoltMeshMaker.PeturbVerticesRandomly(str);
            AdeptusLightningBoltMeshMaker.DoubleVertices();
            return AdeptusLightningBoltMeshMaker.MeshFromVerts();
        }

        // Token: 0x0600137D RID: 4989 RVA: 0x00095543 File Offset: 0x00093943
        public static Mesh NewBoltMesh(Vector2 vector, float str = 3f)
        {
            AdeptusLightningBoltMeshMaker.lightningTop = vector;
            AdeptusLightningBoltMeshMaker.MakeVerticesBase();
            AdeptusLightningBoltMeshMaker.PeturbVerticesRandomly(str);
            AdeptusLightningBoltMeshMaker.DoubleVertices();
            return AdeptusLightningBoltMeshMaker.MeshFromVerts();
        ...
        {
            int num = (int)Math.Ceiling((double)((Vector2.zero - AdeptusLightningBoltMeshMaker.lightningTop).magnitude / 0.25f));
            Vector2 b = AdeptusLightningBoltMeshMaker.lightningTop / (float)num;
            AdeptusLightningBoltMeshMaker.verts2D = new List<Vector2>();
            Vector2 vector = Vector2.zero;
            for (int i = 0; i < num; i++)
            {
                AdeptusLightningBoltMeshMaker.verts2D.Add(vector);
                vector += b;
            }
        }
        ...
        private static void DoubleVertices()
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                AdeptusLightningBoltMeshMaker.verts2D.Add(item);
                AdeptusLightningBoltMeshMaker.verts2D.Add(item2);
            }
        }
        ...
        {
            Vector3[] array = new Vector3[AdeptusLightningBoltMeshMaker.verts2D.Count];
            for (int i = 0; i < array.Length; i++)
            {
                array[i] = new Vector3(AdeptusLightningBoltMeshMaker.verts2D[i].x, 0f, AdeptusLightningBoltMeshMaker.verts2D[i].y);
            }
            float num = 0f;
            Vector2[] array2 = new Vector2[AdeptusLightningBoltMeshMaker.verts2D.Count];
            for (int j = 0; j < AdeptusLightningBoltMeshMaker.verts2D.Count; j += 2)
            {
                array2[j] = new Vector2(0f, num);
                array2[j + 1] = new Vector2(1f, num);
                num += 0.04f;
            }
            int[] array3 = new int[AdeptusLightningBoltMeshMaker.verts2D.Count * 3];
            for (int k = 0; k < AdeptusLightningBoltMeshMaker.verts2D.Count - 2; k += 2)
            {
                int num2 = k * 3;
                array3[num2] = k;
                array3[num2 + 1] = k + 1;
