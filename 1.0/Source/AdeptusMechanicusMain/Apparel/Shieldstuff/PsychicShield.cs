...
namespace AdeptusMechanicus
{
    ...
    public class PsychicShield : Apparel
    {
        ...
        public float EnergyMax
        {
            get
            {
                float psychicpower = Wearer != null ? Wearer.GetStatValue(StatDefOf.PsychicSensitivity) : 0f;
                return Math.Max(0, this.GetStatValue(StatDefOf.EnergyShieldEnergyMax, true) * psychicpower);
            }
        }
        ...
        public float EnergyGainPerTick
        {
            get
            {
                if (Wearer != null && Wearer.isPsyker(out int Level, out float Mult))
                {
                    return Math.Max(0, ((this.GetStatValue(StatDefOf.EnergyShieldRechargeRate, true) * Mult) / 60f) );
                }
                return Math.Max(0, (this.GetStatValue(StatDefOf.EnergyShieldRechargeRate, true) / 60f));
            }
        }
        ...
        private bool ShouldDisplay
        {
            get
            {
                ...
                return wearer.Spawned && !wearer.Dead && !wearer.Downed && Wearer.isPsyker() && (wearer.InAggroMentalState || wearer.Drafted || (wearer.Faction.HostileTo(Faction.OfPlayer) && !wearer.IsPrisoner) || Find.TickManager.TicksGame < this.lastKeepDisplayTick + this.KeepDisplayingTicks);
            }
        }
