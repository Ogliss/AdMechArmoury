...
namespace AdeptusMechanicus
{
    ...
    {
        private static CompAbilityUser cau;
        private static CompAbilityItem cai;
...
//                Log.Message("------------------");
        ...
        public static void Notify_EquipmentAdded_PostFix(Pawn_EquipmentTracker __instance, ThingWithComps eq)
        {
                ...
                foreach (var cai in eq.GetComps<CompAbilityItem>()) //((Pawn)__instance.ParentHolder).GetComps<CompAbilityItem>() )
        ...
        public static void Notify_ApparelAdded_PostFix(Pawn_ApparelTracker __instance, Apparel apparel)
        {
            ...
            ) //((Pawn)__instance.ParentHolder).GetComps<CompAbilityItem>() ) CompAbilityUser
                foreach (var cau in ((Pawn)__instance.ParentHolder).GetComps<CompAbilityUser>())
                if (cau.GetType() == cai.Props.AbilityUserClass)
                    {
                        cai.AbilityUserTarget = cau;
        ...
        */
        ...
        public static void Notify_ApparelAdded_PostFix(Pawn_ApparelTracker __instance, Apparel apparel)
        {
            Log.Message("null check 1");
            if (apparel?.GetComps<CompAbilityItem>()?.Count() > 0)
            {
                int? compcount = apparel?.GetComps<CompAbilityItem>()?.Count();
                string logroll = string.Format("Cur CompAbilityItem count: {0}", compcount);
                Log.Message(logroll);
                Log.Message("null check 1 passed");
                foreach (var cai in apparel.GetComps<CompAbilityItem>()) ; //((Pawn)__instance.ParentHolder).GetComps<CompAbilityItem>() ) CompAbilityUser
            }

            Log.Message("null check 2");
            if (((Pawn)__instance.ParentHolder)?.GetComps<CompAbilityUser>()?.Count() > 0)
            {
                int? compcount = ((Pawn)__instance.ParentHolder)?.GetComps<CompAbilityUser>()?.Count();
                string logroll = string.Format("Cur CompAbilityUser count: {0}", compcount);
                Log.Message(logroll);
                Log.Message("null check 2 passed");
                foreach (var cau in ((Pawn)__instance.ParentHolder).GetComps<CompAbilityUser>()) ;
            }

            Log.Message("null check 3");
            if (cau.GetType() == cai.Props.AbilityUserClass)
            {
                cai.AbilityUserTarget = cau;
                if (cai?.Props.Abilities?.Count > 0)
                {
                    int? compcount = cai?.Props.Abilities?.Count;
                    string logroll = string.Format("Cur Abilities count: {0}", compcount);
                    Log.Message("null check 3 passed");
                    foreach (var abdef in cai.Props.Abilities) cau.AddApparelAbility(abdef);
                }
            }
        }
