...
namespace AdeptusMechanicus
{
    ...
    public static class AdeptusMeshMaker
    {
        ...
        {
            Rand.PushState();
            AdeptusMeshMaker.lightningTop = new Vector2(Rand.Range(-0.2f, 0.2f), distance);
            Rand.PopState();
            AdeptusMeshMaker.MakeVerticesBase();
            if (amplitude > 0f)
            {
                AdeptusMeshMaker.PeturbVerticesRandomly(amplitude);
            }
            AdeptusMeshMaker.DoubleVertices();
            return AdeptusMeshMaker.MeshFromVerts();
        }

        // Token: 0x06000760 RID: 1888 RVA: 0x00067118 File Offset: 0x00065318
        private static void MakeVerticesBase()
        {
            int num = (int)Math.Ceiling((double)((Vector2.zero - AdeptusMeshMaker.lightningTop).magnitude / 0.25f));
            Vector2 b = AdeptusMeshMaker.lightningTop / (float)num;
            AdeptusMeshMaker.verts2D = new List<Vector2>();
            Vector2 vector = Vector2.zero;
            for (int i = 0; i < num; i++)
            {
                AdeptusMeshMaker.verts2D.Add(vector);
                vector += b;
            }
        }
        ...
        private static void PeturbVerticesRandomly(float amplitude)
        {
            ...
            Rand.PopState();
            List<Vector2> list = AdeptusMeshMaker.verts2D.ListFullCopy<Vector2>();
            AdeptusMeshMaker.verts2D.Clear();
            for (int i = 0; i < list.Count; i++)
            {
                float d = amplitude * (float)perlin.GetValue((double)i, 0.0, 0.0);
                Vector2 item = list[i] + d * Vector2.right;
                AdeptusMeshMaker.verts2D.Add(item);
            }
        }
        ...
        private static void DoubleVertices()
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                AdeptusMeshMaker.verts2D.Add(item);
                AdeptusMeshMaker.verts2D.Add(item2);
            }
        }
        ...
        {
            Vector3[] array = new Vector3[AdeptusMeshMaker.verts2D.Count];
            for (int i = 0; i < array.Length; i++)
            {
                array[i] = new Vector3(AdeptusMeshMaker.verts2D[i].x, 0f, AdeptusMeshMaker.verts2D[i].y);
            }
            float num = 0f;
            Vector2[] array2 = new Vector2[AdeptusMeshMaker.verts2D.Count];
            for (int j = 0; j < AdeptusMeshMaker.verts2D.Count; j += 2)
            {
                array2[j] = new Vector2(0f, num);
                array2[j + 1] = new Vector2(1f, num);
                num += 0.04f;
            }
            int[] array3 = new int[AdeptusMeshMaker.verts2D.Count * 3];
            for (int k = 0; k < AdeptusMeshMaker.verts2D.Count - 2; k += 2)
            {
                int num2 = k * 3;
                array3[num2] = k;
                array3[num2 + 1] = k + 1;
