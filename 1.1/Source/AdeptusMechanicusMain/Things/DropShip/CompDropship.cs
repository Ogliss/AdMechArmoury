...
{
    public class CompProperties_Dropship : CompProperties_Transporter
    {
        public CompProperties_Dropship()
        {
            this.compClass = typeof(CompDropship);
        }
        public bool requiresPilot = false;
        public bool alwaysLaunchable = false;
    ...
    public class CompDropship : CompTransporter
    {
        public int dustoffdelay = 120;
        public bool autodustoff = false;
        public bool dustingOff;

        public new CompProperties_Dropship Props => base.Props as CompProperties_Dropship;

        public new bool LoadingInProgressOrReadyToLaunch
        {
            get
            {
                if (Props.alwaysLaunchable)
                {
                    return true;
                }
                if (Props.requiresPilot)
                {
                    if (!this.Transporter.innerContainer.Any(x => x.GetType() == typeof(Pawn) && ((Pawn)x).story != null))
                    {
                        return false;
                    }
                }
                return base.LoadingInProgressOrReadyToLaunch || this.autodustoff;
            }
        }
        ...
        public CompRefuelable Refuelable
        {
            get
            {
                if (this.cachedCompRefuelable == null)
                {
                    this.cachedCompRefuelable = this.parent.GetComp<CompRefuelable>();
                }
                return this.cachedCompRefuelable;
            }
        }
        ...
        public override IEnumerable<Gizmo> CompGetGizmosExtra()
        {
            ...
            command_Action.action = delegate ()
            {
                ...
                MapParent mapparent = this.parent.Map.Parent;
                list2.Add(new FloatMenuOption("Local flight", delegate
                {
                    this.StartChoosingDestination(true);
                }, MenuOptionPriority.Default, null, null, 0f, null, null));
                list2.Add(new FloatMenuOption("World flight", delegate ()
                {
        ...
        private void StartChoosingDestination(bool localOnly = false)
        {
            if (localOnly)
            {
                MapParent mapparent = this.parent.Map.Parent;
                ...
                Find.Targeter.BeginTargeting(TargetingParameters.ForDropPodsDestination(), delegate (LocalTargetInfo x)
                {
                    this.TryLaunch(mapparent.Tile, new TransportPodsArrivalAction_LandInSpecificCell(mapparent, x.Cell), null);
                }, null, delegate
                {
            ...
            */
            ...
            if (cafr == null)
            {
             ...
             //   Building fuelingPortSource = this.FuelingPortSource;//compTransporter.Launchable.FuelingPortSource;
                ...
                Thing dropship = this.parent;
                IntVec3 poc = dropship.Position;
            ...
            //    directlyHeldThings.TryAddOrTransfer(dropship);
                ...
                this.dustingOff = true;
                dropship.DeSpawn();
                activeDropPod.Contents.innerContainer.TryAddOrTransfer(dropship, 1, false);
            ...
            //    CompUSCMDropship dropship1 = null;
                ...
                */
                ...
                TravelingTransportPods travelingTransportPods = (TravelingTransportPods)WorldObjectMaker.MakeWorldObject(DefDatabase<WorldObjectDef>.GetNamed(this.parent.def.defName + "_Traveling", true));
        ...
        private void CheckAutoload()
        {
            ...
            if (CompDropship.tmpRequiredItems.Any<ThingDefCount>() || CompDropship.tmpRequiredPawns.Any<Pawn>())
            {
                ...
                CompDropship.tmpAllSendablePawns.AddRange(TransporterUtility.AllSendablePawns_NewTmp(this.TransportersInGroup, this.parent.Map, false));
                CompDropship.tmpAllSendableItems.Clear();
                CompDropship.tmpAllSendableItems.AddRange(TransporterUtility.AllSendablePawns_NewTmp(this.TransportersInGroup, this.parent.Map, false));
