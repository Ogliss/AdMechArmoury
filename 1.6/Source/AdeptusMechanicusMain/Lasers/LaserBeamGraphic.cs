...
namespace AdeptusMechanicus.Lasers
{
    ...
    public class LaserBeamGraphic : Thing
    {
        ...
        public override void ExposeData()
        {
            ...
            Scribe_References.Look(ref projectile, "projectile", true);
        }
#endregion Vars
        ...
        public void Setup(Thing launcher, Vector3 origin, Vector3 destination, Verb verb = null, Thing hitThing = null, Effecter effecter = null, EffecterDef effecterDef = null, Projectile projectile = null)
        {
            //SetColor(launcher);
        ...
        public override void SpawnSetup(Map map, bool respawningAfterLoad)
        {
            ...
            {
                float scale = decoration.scale > 0 ? decoration.scale : projDef.lightningArcWidthDistCurve?.Evaluate(Lifetime) ?? projDef.beamWidth;
                float spacing = decoration.spacing * scale;
                float initalOffset = decoration.initialOffset * scale;
        ...
        protected virtual void Explode()
        {
            ...
            if (this.projDef.projectile.explosionEffect != null)
            {
                Effecter effecter = this.projDef.projectile.explosionEffect.Spawn();
                effecter.Trigger(new TargetInfo(intVec, map, false), new TargetInfo(intVec, map, false));
                effecter.Cleanup();
            }
            IntVec3 center = intVec;
            Map map2 = map;
            float explosionRadius = this.projDef.projectile.explosionRadius;
            DamageDef damageDef = this.projDef.projectile.damageDef;
            Thing launcher = this.launcher;
            int damageAmount = this.projectile?.DamageAmount ?? this.projDef.projectile.GetDamageAmount(1f, null);
            float aP = this.projectile?.ArmorPenetration ?? this.projDef.projectile.GetArmorPenetration(null);
            SoundDef soundExplode = this.projDef.projectile.soundExplode;
            ThingDef equipmentDef = this.equipmentDef;
            ThingDef def = this.projDef;
            ThingDef postExplosionSpawnThingDef = this.projDef.projectile.postExplosionSpawnThingDef;
            float postExplosionSpawnChance = this.projDef.projectile.postExplosionSpawnChance;
            int postExplosionSpawnThingCount = this.projDef.projectile.postExplosionSpawnThingCount;
            ThingDef preExplosionSpawnThingDef = this.projDef.projectile.preExplosionSpawnThingDef;
            GenExplosion.DoExplosion(center, map2, explosionRadius, damageDef, launcher, damageAmount, aP, soundExplode, equipmentDef, def, null, postExplosionSpawnThingDef, postExplosionSpawnChance, postExplosionSpawnThingCount, this.projDef.projectile.postExplosionGasType, null, 255, this.projDef.projectile.applyDamageToExplosionCellsNeighbors, preExplosionSpawnThingDef, this.projDef.projectile.preExplosionSpawnChance, this.projDef.projectile.preExplosionSpawnThingCount, this.projDef.projectile.explosionChanceToStartFire, this.projDef.projectile.explosionDamageFalloff);
        }
