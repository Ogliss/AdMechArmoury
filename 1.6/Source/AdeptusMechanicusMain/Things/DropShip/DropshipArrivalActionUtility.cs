...
namespace AdeptusMechanicus
{
    public static class DropShipArrivalActionUtility
    {
        ...
        public static IEnumerable<FloatMenuOption> GetFloatMenuOptions<T>(Func<FloatMenuAcceptanceReport> acceptanceReportGetter, Func<T> arrivalActionGetter, string label, CompDropship representative, int destinationTile, Caravan car) where T : TransportersArrivalAction
        {
        ...
        public static IEnumerable<FloatMenuOption> GetVisitFloatMenuOptions(CompDropship representative, IEnumerable<IThingHolder> pods, Settlement settlement, Caravan car)
        {
            return DropShipArrivalActionUtility.GetFloatMenuOptions<TransportersArrivalAction_VisitSettlement>(() => TransportersArrivalAction_VisitSettlement.CanVisit(pods, settlement),
                () => new TransportersArrivalAction_VisitSettlement(settlement,"MessageShuttleArrived"), "VisitSettlement".Translate(settlement.Label), representative, settlement.Tile, car);
        }
    }
}
