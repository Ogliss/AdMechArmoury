    ...
    public class OrbitalBombardment : OrbitalStrike
	{
		...
		private void EffectTick()
		{
			...
			for (int i = this.projectiles.Count - 1; i >= 0; i--)
			{
				...
				if (this.projectiles[i].LifeTime <= 0)
				{
					...
					int damAmount = this.projectiles[i].ordnance.projectile.GetDamageAmount(this.def, null);
					...
					GenExplosion.DoExplosion(targetCell, map, randomInRange, bomb, instigator, damAmount, armorPenetration, explosionSound, this.weaponDef, def, null, null, 0f, 1, null, null, 255, false, null, 0f, 1, 0f, false, null, null);
					this.projectiles.RemoveAt(i);
				}
			}
		}
		...
		private void StartRandomFire()
		{
			...
			FireUtility.TryStartFireIn((from x in GenRadial.RadialCellsAround(base.Position, (float)this.randomFireRadius, true)
										...
										select x).RandomElementByWeight((IntVec3 x) => OrbitalBombardment.DistanceChanceFactor.Evaluate(x.DistanceTo(base.Position))), base.Map, Rand.Range(0.1f, 0.925f), this);
