...
namespace AdeptusMechanicus
{
    ...
    public partial class AdvancedVerbProperties : VerbProperties, IMuzzlePosition, IAdvancedVerb
    {
        ...
        public float scattershotSpread = 0.01f;
        public float scattershotSpreadPerCell = 1f;
        ...
        private int scattershotCountInt = -1;
        public int ScattershotCount
        {
            get
            {
                if (scattershotCountInt == -1)
                {
                    scattershotCountInt = defaultProjectile?.GetModExtensionFast<ScattershotProjectileExtension>()?.projectileCount ?? scattershotCount;
                }
                return scattershotCountInt;
            }
        }
        public float scattershotSpreadInt = -1f;
        public float ScattershotSpread
        {
            get
            {
                if (scattershotSpreadInt == -1f)
                {
                    scattershotSpreadInt = defaultProjectile?.GetModExtensionFast<ScattershotProjectileExtension>()?.spread ?? scattershotSpread;
                }
                return scattershotSpreadInt;
            }
        }
        public float scattershotSpreadPerCellInt = -1f;
        public float ScattershotSpreadPerCell
        {
            get
            {
                if (scattershotSpreadPerCellInt == -1f)
                {
                    scattershotSpreadPerCellInt = defaultProjectile?.GetModExtensionFast<ScattershotProjectileExtension>()?.perCells ?? scattershotSpreadPerCell;
                }
                return scattershotSpreadPerCellInt;
            }
        }
