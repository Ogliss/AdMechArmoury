            ...
            if (hitPawn != null && !hitPawn.RaceProps.Humanlike)
            {
                if (hitPawn.ageTracker.CurKindLifeStage is SwarmKindLifeStage swarm)
                {
                    if (!swarm.subStagesHealth.NullOrEmpty())
                    {
                        ...
                        if (ind > 0) hitPawn.Drawer.renderer.BodyGraphic = swarm.subStagesHealth[ind - 1].bodyGraphicData.Graphic;
                    }
                    ...
                    if (cloneSource.Weapon != null)
                    {
                        ...
                        if (WeaponRules != null || compForce != null)
                        {
                            if (WeaponRules?.ForceWeapon ?? compForce != null)
                            {
                                ...
                                bool casterPsychiclySensitive = Caster.RaceProps.Humanlike && (Caster.story.traits.HasTrait(AdeptusTraitDefOf.PsychicSensitivity) || Caster.story.traits.HasTrait(DefDatabase<TraitDef>.GetNamedSilentFail("Psyker")));
                                bool Activate = false;
                                if ((casterPsychiclySensitive || !requiresPsyker) && target.def.category == ThingCategory.Pawn && target is Pawn Victim)
                                {
                                    int casterPsychiclySensitiveDegree = casterPsychiclySensitive ? Caster.story.traits.DegreeOfTrait(AdeptusTraitDefOf.PsychicSensitivity) : 0;
                                    if ((casterPsychiclySensitiveDegree >= 1 || !requiresPsyker))
                                    {
                                        ...
                                        bool targetPsychiclySensitive = Victim.RaceProps.Humanlike && Victim.story.traits.HasTrait(AdeptusTraitDefOf.PsychicSensitivity);
                                        ...
                                        {
                                            int targetPsychiclySensitiveDegree = Victim.story.traits.DegreeOfTrait(AdeptusTraitDefOf.PsychicSensitivity);
                                        ...
                                        else { /*int targetPsychiclySensitiveDegree = 0;*/ }
                                        {
                                            ...
                                            if (Activate)
                                            {
                                                ...
                                                Thing thing = target;
                                                GenExplosion.DoExplosion(position, map2, explosionRadius, damDef, launcher, (int)damAmount, armorPenetration, soundExplode, source, null, thing, null, 0f, 0, null, null, 255, false, null, 0, 0, 0, false);
