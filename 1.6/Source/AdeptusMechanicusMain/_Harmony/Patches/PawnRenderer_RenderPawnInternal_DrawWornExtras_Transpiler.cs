        ...
        */
        ...
        //    MethodInfo drawApparel = AccessTools.Method(typeof(PawnRenderer), )
            for (int i = 0; i < instructionList.Count; i++)
            {
                ...
                if (i > 1 && instructionList[index: i].OperandIs(AccessTools.Method(type: typeof(PawnRenderer), name: nameof(PawnRenderer.ShamblerScarDrawer))) && (i+1) < instructionList.Count)
                {
        ...
        //    Log.Message(string.Concat(new string[]
            ...
            */
            ...
            Vector2 defaultSize = mesh?.bounds.size ?? (portrait ? pawn.Drawer.renderer.HeadGraphic.MeshAt(bodyFacing).bounds.size : pawn.Drawer.renderer.BodyGraphic.MeshAt(bodyFacing).bounds.size);
            Vector2 size = mesh?.bounds.size ?? (portrait ? pawn.Drawer.renderer.HeadGraphic.MeshAt(bodyFacing).bounds.size : pawn.Drawer.renderer.BodyGraphic.MeshAt(bodyFacing).bounds.size);
            if (pawn.RaceProps.Humanlike)
            {
