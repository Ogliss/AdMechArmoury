...
{
    public static class FactionSpawningUtility
    {
        ...
        private static FactionRelationKind GetFactionKind(Faction faction, bool firstOfKind)
        {
            ...
            if (faction.def.CanEverBeNonHostile)
            {
                ...
                {
                    if (startingGoodwill(faction) > 0) result = FactionRelationKind.Neutral;
                }
            }

            return result;
        }
        private static float startingGoodwill(Faction a)
        {
            Faction b = Faction.OfPlayer;
            bool permanentEnemy = a.def.permanentEnemy;
            int result;
            if (permanentEnemy)
            {
                result = -100;
            ...
            {
                bool flag = a.def.permanentEnemyToEveryoneExceptPlayer;
                if (flag)
                {
                    result = -100;
                }
                ...
                {
                    bool flag2 = a.def.permanentEnemyToEveryoneExcept != null && !a.def.permanentEnemyToEveryoneExcept.Contains(b.def);
                    if (flag2)
                    {
                        result = -100;
                    }
                    ...
                    {
                        bool naturalEnemy = a.def.naturalEnemy;
                        if (naturalEnemy)
                        {
                            result = -80;
                        }
        ...
        private static void CreateSettlements(Faction faction, int amount, int minDistance, out int spawned)
        {
            ...
            var tilesInDistance = tiles.Where(x=> IsFarEnoughFromPlayer(x)).Take(amount).ToArray();
