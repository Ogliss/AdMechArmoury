        ...
        public override void DrawAt(Vector3 drawLoc, bool flip = false)
        {
            if (this.innerContainer.Any)
            {
                foreach (var item in innerContainer)
                {
                    ...
                    if (flyer != null)
                    {
                        if (flyer.GetThingForGraphic() is Thing CarriedThing)
                        {
                            if (CarriedThing is Pawn FlyingPawn)
                            {
                                Verse.PawnRenderUtility.DrawCarriedThing(FlyingPawn, drawLoc, CarriedThing);
                                return;
                            }
                        }
