...
namespace AdeptusMechanicus
{
    ...
    public class ReserveForce : IExposable
    {
        ...
        public ReserveForce(Faction faction = null, Lord lord = null, int delay = 0, Map map = null, List<Pawn> members = null)
        {
        ...
        public string TimeBeforeDeployment()
        {
            ...
            {
                result = delay.TicksToSeconds().ToStringByStyle(ToStringStyle.FloatTwo).ToString();
            }
            return result;
        }
        ...
        //    Log.Message($"DeployReserve PRE: Tunnellers: {tunnellers.Count}");
            ...
            else if (!Lords.NullOrEmpty())
            {
                l = Lords.RandomElement();
            }
            if (l != null)
            {
                Log.Message($"DoStrike Adding {pawns.Count} to Lord {l}");
        ...
        public void Arrive(List<Pawn> pawns, ReserveDeploymentType strikeType = ReserveDeploymentType.DropPod)
        {
            for (int i = 0; i < pawns.Count; i++)
            {
                ...
                if (target != IntVec3.Invalid)
                    DropCellFinder.TryFindDropSpotNear(target, map, out dropCenter, false, true);
                else if (strikeType != ReserveDeploymentType.Infiltrate)
                    DropCellFinder.TryFindRaidDropCenterClose(out dropCenter, map);
                else
                {
                    if (DropCellFinder.TryFindRaidDropCenterClose(out dropCenter, map))
                    {
                        RCellFinder.TryFindRandomSpotJustOutsideColony(dropCenter, map, out dropCenter);
                    }
                    else
                    {
                ...
                {
                    Log.Warning("Failed to find target location for deep strike");
                    this.delay = 400;
                }
                else if (strikeType == ReserveDeploymentType.Infiltrate)
